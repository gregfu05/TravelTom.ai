...
def upgrade() -> None:
    ...
    op.create_index("gin_catalog_tags", "catalog_items", ["tags"], postgresql_using="gin")

    op.create_table(
        "embeddings",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("item_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("catalog_items.id"), nullable=False),
    ...
    op.create_index("idx_events_idempotency", "events", ["idempotency_key"], unique=True)
