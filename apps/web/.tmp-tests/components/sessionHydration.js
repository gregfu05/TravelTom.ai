const TRIVIAL_GREETING_PATTERN = /^\s*(?:hello|hi|hey|yo|howdy|greetings)\b(?:[\s,.!?:-]*(?:tom|travel\s*tom|traveltom))?[\s,.!?:-]*$/i;
...
function hasMeaningfulState(state) {
    ...
    if (isRecord(constraints)) {
        if (hasMeaningfulValue(constraints.origin) ||
    ...
    if (isRecord(preferences)) {
        if (hasMeaningfulValue(preferences.weighted_interests) ||
    ...
    if (isRecord(conversation)) {
        if (hasMeaningfulValue(conversation.last_recommendation_item_type) ||
...
export function shouldDiscardHydratedSession(session) {
