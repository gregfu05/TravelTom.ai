...
def main():
    ...
    df_other_match = df_other[
        df_other["city"].str.lower().str.strip().isin(target_names_lower)
    ]
    ...
    tbo_with_stars = (
        tbo_mask & df["stars"].notna() & (df["stars"] >= 1) & (df["stars"] <= 5)
    )
    ...
    for etype in df["entity_type"].unique():
        ...
        median = df.loc[
            (df["entity_type"] == etype) & df["stars"].notna(), "stars"
        ].median()
